...
Перед отдачей результатов через `/assets/...` или выгрузкой в CDN убедитесь, что в каталоге `output/` нет посторонних или чувствительных данных. Быстрая ручная проверка:

```bash
ls -lh output/            # общий список
find output -maxdepth 2 -type f
```

Хранилище плоское: файлы изображений и JSON‑метаданные лежат прямо в `output/`, а служебные данные — в `output/data/`. Просмотрите свежие файлы (по дате или префиксу имени генерации), убедитесь в корректности картинок/PDF и удалите лишнее перед публикацией.
...
2. **Есть ли файлы в `output/`?** На сервере выполните `ls -lh output/ output/data/`. При успешных ответах SDK картинки и JSON‑метаданные появляются тут. Пустой каталог — признак того, что генерации не дошли до сохранения (ошибка сети, неверный ключ, квота закончилась).
3. **Состояния в `output/data/generations.json`**. Если файл существует, посмотрите последние записи: статус `error` означает ошибку от Gemini, `generating`/`regenerating` — активный запрос. При перезапуске приложения файл перечитывается, так что накопившиеся ошибки останутся видны.
...
5. **Доступность статики `/assets/...`**. Убедитесь, что в nginx или другом прокси есть блок `location /assets/ { proxy_pass http://127.0.0.1:8000; }`. Без него файлы из `output/` не отдаст фронтенду даже если они существуют.
