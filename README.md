...
Перед отдачей результатов через `/assets/...` или выгрузкой в CDN зайдите в каталог `output/` и убедитесь, что там только ожидаемые файлы генераций. Быстрая ручная проверка:

```bash
ls -lh output/
```

Каждая генерация лежит в своём подкаталоге (`output/<generation_id>/`). Проверьте, что изображения, PDF и вспомогательные файлы выглядят корректно и не содержат конфиденциальных данных перед публикацией.
